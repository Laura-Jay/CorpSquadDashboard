      ...
      case "startDate":
        {
          ...
          filteredProjects = projectsArr.filter((project) => {
            return (
              datesArrStart[0] <= new Date(project.project.startDate) &&
              new Date(project.project.startDate) <= datesArrStart[1]
            );
          });
        }
        break;
      case "endDate":
        {
          ...
          filteredProjects = projectsArr.filter(
            (project) =>
              datesArrEnd[0] <= new Date(project.project.endDate) &&
              new Date(project.project.endDate) <= datesArrEnd[1]
          );
        }
        break;
      case "size":
        filteredProjects = projectsArr.filter(
          (project) =>
            parseInt(filterVal) - 10000 <= parseFloat(project.project.size) &&
            parseFloat(project.project.size) <= parseInt(filterVal)
        );
        break;
      case "employee":
